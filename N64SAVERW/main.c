...
int choice = 0;
int num_options = 5;
char* options[] = { "Read Save", "Write to SD", "Read from SD", "Erase Save", "Write Save"};
...
void printmenu(char* title, char* items[], int num_items, int choice, int type) {
  ...
  if (type == 6) { printf("CPAK "); printf("%d", (CPAK_CONTROLLER + 1)); }
  printf("\n");
  printf("\n");
  printf("\n");
  printf("Use DPAD and A to select Option\n");
  printf("Press Z to change Save Type\n");
  if (type == 6) printf("Press L to change Controller\n");
  printf("\n");
...
void readcpak(void) {
  ...
  if (identify_accessory(CPAK_CONTROLLER) == ACCESSORY_MEMPAK) {
    printf("Reading 256K CPAK into Buffer 3\n");
    for ( int j = 0; j < 128; j++ ) {
      read_mempak_sector( CPAK_CONTROLLER, j, &buffer3[j * MEMPAK_BLOCK_SIZE]  );
    }
    printf("Waiting 1 second...\n");
    delay(1000);
    ...
    for ( int j = 0; j < 128; j++ ) {
      read_mempak_sector( CPAK_CONTROLLER, j, &buffer1[j * MEMPAK_BLOCK_SIZE]  );
    }
    delay(20);
...
void writecpak(void) {
  ...
  if (identify_accessory(CPAK_CONTROLLER) == ACCESSORY_MEMPAK) {
    ...
    for ( int j = 0; j < 128; j++ ) {
      write_mempak_sector( CPAK_CONTROLLER, j, &buffer3[j * MEMPAK_BLOCK_SIZE]  );
    }
...
void erasecpak(void) {
  ...
  if (identify_accessory(CPAK_CONTROLLER) == ACCESSORY_MEMPAK) {
    ...
    for ( int j = 0; j < 128; j++ ) {
      write_mempak_sector( CPAK_CONTROLLER, j, &buffer4[j * MEMPAK_BLOCK_SIZE]  );
    }
  ...
  */
  ...
  while (1) {
    ...
    while (1) {
      ...
      // if L is pressed change CPAK controller
      if (keys.c[DEFAULT_CONTROLLER].L) {
        if ( CPAK_CONTROLLER == 0)
          CPAK_CONTROLLER = 1;
        else if ( CPAK_CONTROLLER == 1)
          CPAK_CONTROLLER = 2;
        else if ( CPAK_CONTROLLER == 2)
          CPAK_CONTROLLER = 3;
        else if ( CPAK_CONTROLLER == 3)
          CPAK_CONTROLLER = 0;
        //print menu to show current selection
        printmenu("N64 Save R/W", options, num_options, choice, type);
      }
